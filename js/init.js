        ...
        if(data.responseCode === 1 && data.response!=null){
            if(data.response.length > 0){
                ...
                for(var i=0; i <data.response.length; i++){                  
                    ...
                    $(cln).find(".desc").text(data.response[i].description);
                    $(cln).find(".desc").append( '</br><span>by '+ data.response[i].full_name  +'</span>');
                    var date = new Date(data.response[i].created_on);
                    $(cln).find(".date").text(date.toDateString().slice(3));
                    $(cln).find(".btn-share").attr("id",data.response[i]._id);
                    var cover_art_url = null;
                    if(data.response[i].track_icon && Array.isArray(data.response[i].track_icon)){
                        $(cln).find("#icon").attr("src","data:image/png;base64,"+ data.response[i].track_icon[0].buffer);
                        cover_art_url = "data:image/png;base64,"+ data.response[i].track_icon[0].buffer;
                    }else if(data.response[i].track_icon){
                        $(cln).find("#icon").attr("src", data.response[i].track_icon);
                        cover_art_url = data.response[i].track_icon;
                    }
                   // console.log("PODCAST.user.sessionId>>" + PODCAST.user.sessionId);
                   console.log(data.response[i].url);
                    var track = {
                        "name": data.response[i].filename,
                        "artist": data.response[i].description + '</br><span>by '+ data.response[i].full_name  +'</span>',
                        "url": data.response[i].url,//"/api/singleTrack/"+data.response[i]._id+"/"+PODCAST.user.sessionId,
                        "live":true,
